...
| Complete Standard/Expert bypass | Required                      | Required           | Off by default; all valid configured scope skips SiteWipe review; preflight precedes one **Clean now** activation; native target prompt may appear |
...
| Associated target               | Required                      | Smoke              | Detailed mode explicitly acknowledges displayed scope; bypass includes only configured preflight-bound scope without displaying it                 |
