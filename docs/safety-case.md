...
The P0 consent defect is closed in the current source by removing the bypass rather than approving it. MIT and the recorded provenance are owner-approved; they do not authorize publication or provide trademark clearance. The safety argument remains incomplete until the browser matrix, accessibility run, benchmark, exact-public-version/name-collision decisions, authentic media, remote controls, and final artifact evidence are retained. See [ADR 0009](./decisions/0009-mandatory-cleanup-review.md), the [historical superseded record](./decisions/0004-complete-review-bypass.md), and [release readiness](./release-readiness.md).
