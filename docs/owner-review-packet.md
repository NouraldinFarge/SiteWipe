...
| Private upload/security setup         | Initial push and branch parity verified; prior candidate `1.11.8` passed exact-head hosted CI and CodeQL with retained zero-finding SARIF; selected GitHub-owned Actions, full-SHA pinning, dependency alerts, and automated fixes are verified; the current exact head and public-only controls remain pending; no independent reviewer is approved | Authorized private controls applied; current hosted rerun/public-only controls deferred | Proves the private workflow mechanism while preserving separate authorization for visibility, merging, tags, releases, Pages, and the final public transition; required review must wait for a second owner-approved, write-capable reviewer |
