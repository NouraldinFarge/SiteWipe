...
- 55–70s: show unrelated control tenant unchanged plus permission/DNR recovery state;
- 70–85s: show privacy controls, report expiry/forget, and the no-complete-erasure limitation.

Record exact browser/OS version, artifact name/SHA-256, fixture version/seed, profile path classification (not the private path itself), timestamp, and every assertion. Media approval cannot substitute for the browser evidence record.
