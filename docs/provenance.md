...
- [x] The Git worktree root is the nested SiteWipe candidate, the outer container is not a Git worktree, one owner-approved fetch/push destination is configured, and the Git-visible candidate set exactly equals the declared source closure. Unresolved index stages, symlinks, submodules, nested Git metadata, prohibited private/generated paths, non-portable paths, and case-insensitive collisions fail closed.
...
The publication gate consumes `docs/evidence/provenance-audit.json`. Its technical and owner-provenance fields pass, including the Git/source publication-closure invariant enforced by `npm run check:publication-scope`. That does not approve an exact public version, installed-browser claims, media, remote controls, public visibility, a release, a store submission, or professional-profile publication.
