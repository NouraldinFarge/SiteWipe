...
`SiteWipe` is the owner-approved custom product identity, and MIT is the owner-approved first-party project license. Known exact-name browser-extension listings mean uniqueness and legal clearance are not claimed. Version `1.11.29` is public source but remains an unreleased prerelease; it must not be represented as an approved binary or submitted to a browser store before its remaining gates pass.
