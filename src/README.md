...
`SiteWipe` is the owner-approved custom product identity. Known exact-name browser-extension listings mean uniqueness and legal clearance are not claimed. Version `1.11.4` remains a private candidate; no public version or project-level license has been approved, and this candidate must not be published or submitted to a browser store.
